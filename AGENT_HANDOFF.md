...
> Older history. `CLAUDE.md` is the current handoff and wins where they disagree.
...
- A gesture practice tab with live MediaPipe hand-landmark tracking, trained local recognition, and Gemini-first corrective feedback
...
- Activity results are saved to `activity_results` when a play finishes. Gesture practice results are never saved (no gesture attempts).
...
- Gesture practice starts the webcam, draws a live landmark outline over up to two moving hands, captures landmark sequences for the bundled trained gesture model, and shows corrective feedback generated by Gemini with local template fallback. The side reference card is not treated as the required target; feedback is based on the gesture actually recognized. The MediaPipe runtime, WASM files, hand-landmarker model, and MakaLearn gesture model are stored locally in the project.
- Gesture practice has a shape-based safety correction for the bundled model's drink-overprediction path: when the captured finger shape clearly matches "I want to eat food" or "I want to go to toilet", the app corrects a raw drink prediction before displaying it. This does not depend on the selected reference card. Palm-facing eat/toilet attempts still show corrective orientation feedback instead of being marked correct.
- Gesture feedback no longer displays the local template immediately while Gemini is pending. The UI shows animated wave dots first, then displays Gemini feedback, the server-returned template fallback, or the client fallback only after an API failure.
...
- `src/features/gesture/gesture-practice-view.tsx` - webcam preview, live MediaPipe landmark overlay, local recognition result packaging, and separated learner/teacher corrective feedback in Student Mode.
...
- `src/utils/gesture-feedback.ts` - typed issue detection, safe local template fallback, and Gemini response validation for corrective feedback.
- `src/app/api/gesture-feedback/route.ts` - server-only Gemini feedback route that keeps `GEMINI_API_KEY` private and falls back to templates on missing key, timeout, malformed JSON, long text, or unsafe text.
- `scripts/test-gesture-feedback.mjs` - focused unit checks for issue detection, template fallback, and Gemini response validation.
...
These commands passed after the latest gesture recognition correction:
...
- Gesture practice uses real MediaPipe hand tracking and the bundled trained model. The old timer-driven fake demo is gone.
