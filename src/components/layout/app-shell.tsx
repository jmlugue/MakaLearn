...
function AuthenticatedShell({ children }: { children: ReactNode }) {
  ...
  return (
    <>
      ...
      >
        <div className={cn("mx-auto", isStudentMode ? "w-full max-w-none" : "max-w-7xl", isStudentGestureViewport && "h-full min-h-0")}>
          <PageTransition key={pathname} className={isStudentGestureViewport ? "relative h-full min-h-0" : undefined}>
            {children}
          </PageTransition>
        </div>
      </main>
